...
<template>
  <section class="module-workspace">
    <header class="page-header"><h2>活动计时器</h2><div class="summary-chip">今天 <strong>{{ today }}</strong></div></header>
    ...
    <section class="activity-list"><article v-for="entry in visible" :key="entry.id" :class="['activity-card',activityStatus(entry)]"><header><div><span>{{ entry.version || entry.categoryName }}</span><h3>{{ entry.title }}</h3></div><strong>{{ countdown(entry) }}</strong></header><dl><div><dt>开始</dt><dd>{{ entry.rawStart || '待定' }}</dd></div><div><dt>结束</dt><dd>{{ entry.rawEnd || '待定' }}</dd></div><div v-if="entry.floors"><dt>层数</dt><dd>{{ entry.floors }}</dd></div><div v-if="entry.condition" class="activity-description"><dt>更新内容</dt><dd>{{ entry.condition }}</dd></div></dl></article><p v-if="!visible.length" class="state-message">这个分类下没有对应状态的记录，可切换“历史”或“全部”查看。</p></section>
  </section>
</template>
