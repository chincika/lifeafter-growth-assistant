...
填写 `所有者/仓库`、分支和 GitHub Token 后，点击“直接发布到 GitHub”。程序会在二次确认后通过 GitHub Contents API 写入 `releases/`。成功发布后，Token 仅通过 Electron `safeStorage` 绑定 Windows 当前账户加密保存，不写入资料目录、日志、GitHub 仓库或发布包；可随时在维护器中清除。
...
客户端安装包和便携版继续发布到公开仓库；资料维护器可执行文件不得附加到公开 Release。维护器自身版本只发布到私有仓库 `chincika/lifeafter-assistant-maintainer`，不会改变维护器向公开资料仓库写入热更新数据的目标。
...
脚本会打包当前维护器、确认目标仓库确实为 `PRIVATE`、创建或更新对应版本的私有 Release，并比对上传附件的 SHA-256；如果目标仓库是公开仓库会直接拒绝发布。
